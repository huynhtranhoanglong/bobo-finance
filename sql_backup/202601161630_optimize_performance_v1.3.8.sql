-- ============================================================
-- PERFORMANCE OPTIMIZATION (v1.3.8)
-- Date: 2026-01-16
-- Purpose: Optimize database performance with indexes and query improvements
-- ============================================================

-- ============ 1. ADD INDEXES ============
-- Tăng tốc độ lọc theo ngày tháng và user/family cho màn hình History/Dashboard

CREATE INDEX IF NOT EXISTS idx_transactions_filter 
ON transactions(family_id, user_id, date DESC);

CREATE INDEX IF NOT EXISTS idx_transactions_date 
ON transactions(date DESC);

-- ============ 2. OPTIMIZE RLS HELPER ============
-- Mark function as STABLE to allow caching within query execution
...
-- Use Single Pass Aggregation to reduce table scans down to ~2 times
...
BEGIN
    ...
    -- Calculate Min Spend (Must Have) and Std Spend (Must + Nice) for last 90 days
    WITH metrics_agg AS (
        SELECT 
    ...
    -- 3. Monthly Stats (Single Pass Query)
    ...
    IF v_total_debts > 0 THEN v_has_debt := true; END IF;

    -- 4. Get Lists (Wallets, Debts, Funds, Family)
    -- Wallets
    ...
    SELECT json_agg(row_to_json(f)) INTO v_funds FROM (
        SELECT id, name FROM funds WHERE (
            (v_family_id IS NOT NULL AND family_id = v_family_id) OR
            (v_family_id IS NULL AND user_id = v_user_id)
        )
    ) f;
